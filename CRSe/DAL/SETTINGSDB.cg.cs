...
namespace CRSe.CRS.DAL
{
	public partial class SETTINGSDB : DBUtils
	{
		...
		public SETTINGS GetItem(string CURRENT_USER, Int32 CURRENT_REGISTRY_ID, Int32 CRS_SETTINGS_ID)
		{
			...
			try
			{
				...
				sCmd = new SqlCommand("CRS.usp_SETTINGS_getitem", sConn);
		...
		public List<SETTINGS> GetItems(string CURRENT_USER, Int32 CURRENT_REGISTRY_ID)
		{
			...
			try
			{
				...
				sCmd = new SqlCommand("CRS.usp_SETTINGS_getitems", sConn);
				sCmd.CommandTimeout = SqlCommandTimeout;
				sCmd.CommandType = CommandType.StoredProcedure;
				sCmd.Parameters.AddWithValue("@CURRENT_USER", CURRENT_USER);
				sCmd.Parameters.AddWithValue("@CURRENT_REGISTRY_ID", CURRENT_REGISTRY_ID);
				...
				if (objTemp != null  && objTemp.Tables.Count > 0 && objTemp.Tables[0].Rows.Count > 0)
				{
					...
					if (myData != null)
					{
						objReturn = myData.ToList<SETTINGS>();
					}
				}
		...
		public SETTINGS ParseReaderCustom(DataRow row)
		{
			SETTINGS objReturn = new SETTINGS
			{
				COMMENTS = (string)GetNullableObject(row.Field<object>("SETTINGS_COMMENTS")),
				CREATED = (DateTime)GetNullableObject(row.Field<object>("SETTINGS_CREATED")),
				CREATEDBY = (string)GetNullableObject(row.Field<object>("SETTINGS_CREATEDBY")),
				CRS_SETTINGS_ID = (Int32)GetNullableObject(row.Field<object>("SETTINGS_CRS_SETTINGS_ID")),
				DESCRIPTION = (string)GetNullableObject(row.Field<object>("SETTINGS_DESCRIPTION")),
				NAME = (string)GetNullableObject(row.Field<object>("SETTINGS_NAME")),
				STD_REGISTRY_ID = (Int32)GetNullableObject(row.Field<object>("SETTINGS_STD_REGISTRY_ID")),
				UPDATED = (DateTime?)GetNullableObject(row.Field<object>("SETTINGS_UPDATED")),
				UPDATEDBY = (string)GetNullableObject(row.Field<object>("SETTINGS_UPDATEDBY")),
				VALUE = (string)GetNullableObject(row.Field<object>("SETTINGS_VALUE"))
			};
